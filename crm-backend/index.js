 ...
 * @returns {{ name: string, contacts: object[] }} Объект клиента
 */
function makeClientFromData(data) {
  ...
  for (const contact of client.contacts) {
    if (uniqueContacts.has(contact.type)) {
      errors.push({field: 'contacts', message: 'В расписании присутствует нечколько занятий одновременно'});
      break;
    }

    uniqueContacts.add(contact.type);
  }

  if (!client.name) errors.push({ field: 'name', message: 'Не указано название' });
  if (client.contacts.some(contact => !contact.type || !contact.value))
    errors.push({ field: 'contacts', message: 'Расписание заполнено не полностью' });
 ...
 * @returns {{ id: string, name: string, contacts: object[] }[]} Массив клиентов
 */
 ...
 * @returns {{ id: string, name: string, contacts: object[], createdAt: string, updatedAt: string }} Объект клиента
 */
 ...
 * @param {{ name?: string, contacts?: object[] }} data - Объект с изменяемыми данными
 * @throws {ApiError} Клиент с таким ID не найден (statusCode 404)
 * @throws {ApiError} Некорректные данные в аргументе (statusCode 422)
